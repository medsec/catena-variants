...
Phi_Lsb::process(const uint8_t garlic, uint8_t *r, uint8_t* h)
{
	...
	_hashfast->Hash(0, (uint8_t*)r+_graph->index(0, garlic)*H_LEN_FAST, r + _graph->index(R((uint64_t*)(r+H_LEN_FAST*_graph->index(c-1, garlic)), garlic), garlic)*H_LEN_FAST, r+_graph->index(0, garlic)*H_LEN_FAST);
	uint8_t * p = r + _graph->index(0, garlic)*H_LEN_FAST;
	uint8_t * p_old = p;
