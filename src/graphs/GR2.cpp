...
GR2::GR2()
:Registerable("Gray-Reverse 2", "GR2", "As suggested by Ben Harris. Recommended for Lambda up to 4")
{}
...
uint64_t 
GR2::reverse(uint64_t x, const uint8_t n){
	x = bswap_64(x);
...
uint64_t 
GR2::getMemoryRequirement(uint8_t garlic)const
{
	const uint16_t H_LEN_FAST = _hashfast->getHlenFast();
...
uint8_t
GR2::getDefaultGarlic()const{
	return GARLIC;
}


uint8_t
GR2::getDefaulMinGarlic()const{
	return MIN_GARLIC;
}


const uint8_t*
GR2::getDefaultVersionID()const{
	return (const uint8_t*)VERSION_ID.c_str();
}

void 
GR2::H_First(const uint8_t* i1, const uint8_t* i2, uint8_t* hash){
	_hashfast->ResetState();
