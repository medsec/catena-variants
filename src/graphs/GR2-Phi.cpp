...
Registry<GR2_Phi> GR2_Phi;

GR2_Phi::GR2_Phi()
:Registerable("Gray-Reverse 2 Phi", "GR2-Phi", "As suggested by Ben Harris. Recommended for Lambda up to 4")
{}

#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wunused-parameter"
void 
GR2_Phi::process(const uint8_t x[H_LEN], const uint8_t lambda, 
...
uint64_t
GR2_Phi::phi(uint64_t i, uint8_t g)
{	
...
uint64_t 
GR2_Phi::reverse(uint64_t x, const uint8_t n){
	x = bswap_64(x);
...
uint64_t
GR2_Phi::R(uint64_t* input, const uint8_t n){
  return  input[0]>>(64-n);
}

uint64_t 
GR2_Phi::getMemoryRequirement(uint8_t garlic)const
{
	const uint16_t H_LEN_FAST = _hashfast->getHlenFast();
...
uint8_t
GR2_Phi::getDefaultLambda()const{
	return LAMBDA;
}


uint8_t
GR2_Phi::getDefaultGarlic()const{
	return GARLIC;
}


uint8_t
GR2_Phi::getDefaulMinGarlic()const{
	return MIN_GARLIC;
}


const uint8_t*
GR2_Phi::getDefaultVersionID()const{
	return (const uint8_t*)VERSION_ID.c_str();
}

void 
GR2_Phi::H_First(const uint8_t* i1, const uint8_t* i2, uint8_t* hash){
	_hashfast->ResetState();
