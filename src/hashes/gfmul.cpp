...
Registry<GFMul> reggfmul;

GFMul::GFMul()
:Registerable("GF-Multiplication", "GF", 
	"Multiplication in GF2^128 with the GCM-modulus(can uses Intrinsics)")
{}
...
__m128i 
GFMul::sll128(__m128i a, uint8_t count){
	__m128i result,tmp;
...
__m128i 
GFMul::gfmul128(__m128i A, __m128i B)
{
    //A: [A1,A0] B:[B1,B0]
			...
			uint8_t* hash)
{
  ...
  for(int i = 0; i < blocks; i++){
  	left = _mm_load_si128((__m128i*) (i1 + (i*sizeof(__m128i))));
    right = _mm_load_si128((__m128i*) (i2 + (i*sizeof(__m128i))));
    result = gfmul128(left, right);
    _mm_store_si128((__m128i*) (hash + (i*sizeof(__m128i))), result);
  }
}
...
void 
GFMul::clmul64(uint64_t a, uint64_t b, uint64_t* r)
{
...
void 
GFMul::gfmul128(const uint64_t* a, const uint64_t* b, uint64_t* r)
{
    uint64_t e[2];
    uint64_t x[4];
    //Karatsuba
			...
			uint8_t hash[H_LEN])
{
  int blocks = H_LEN/16;
  for(int i = 0; i < blocks; i++){
    gfmul128((uint64_t*)(i1 + (i*16)), (uint64_t*)(i2 + (i*16)),
    	(uint64_t*)(hash + (i*16)));
  }
}
#pragma GCC diagnostic pop
